...
void rst::rasterizer::rasterize_triangle(const Triangle &t, const std::array<Eigen::Vector3f, 3> &view_pos)
{
    ...
    for (int i = box_left; i <= box_right; i++)
        for (int j = box_bottom; j <= box_top; j++)
            if (insideTriangle(i + 0.5, j + 0.5, t.v))
            {
                auto [alpha, beta, gamma] = computeBarycentric2D(i + 0.5, j + 0.5, t.v);
                // todo: Inside your rasterization loop:
                //    * v[i].w() is the vertex view space depth value z.(感觉应该是v[i].z())
                ...
                fragment_shader_payload payload(interpolated_color, interpolated_normal.normalized(), interpolated_texcoords, texture ? &*texture : nullptr); // 定义了一个payload
