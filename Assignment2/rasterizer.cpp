...
{
    auto id = get_next_id();        // 更新一个id
    pos_buf.emplace(id, positions); // 添加一个键值对
...
// 重置颜色缓冲区和深度缓冲区，将颜色缓冲区的每个像素点的颜色设置为黑色，将深度缓冲区的每个像素点的深度设置为无穷大
